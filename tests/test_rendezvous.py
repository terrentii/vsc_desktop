...
async def test_pairing_assigns_roles_and_exchanges_candidates():
    ...
    try:
        room = b"room-abc"
        c1, c2 = RendezvousClient(url), RendezvousClient(url)
...
async def test_relay_forwards_opaque_payload_unchanged():
    ...
    try:
        ...
        first = asyncio.create_task(
            RendezvousClient(url).join(room, [], timeout=2)
        )
        await asyncio.sleep(0.05)
        r2 = await RendezvousClient(url).join(room, [], timeout=2)
        r1 = await first
    ...
    try:
        with pytest.raises(PeerUnavailable):
            await asyncio.gather(
                RendezvousClient(url).join(b"room-1", [], timeout=0.3),
                RendezvousClient(url).join(b"room-2", [], timeout=0.3),
            )
    finally:
        await server.stop()
...
async def test_join_timeout_raises_peer_unavailable():
    ...
    try:
        with pytest.raises(PeerUnavailable):
            await RendezvousClient(url).join(b"lonely", [], timeout=0.2)
    finally:
        await server.stop()
